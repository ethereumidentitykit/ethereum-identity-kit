import { Avatar, LoadingCell } from '../..'
...
const CommonFollowers: React.FC<CommonFollowersProps> = ({ connectedAddress, lookupAddressOrName }) => {
  const { displayedAvatars, displayedNames, resultLength, isLoading } = useCommonFollowers(connectedAddress, lookupAddressOrName)
      ...
      <div className="common-followers-avatars-container">
        {isLoading ? (
          new Array(3).fill(null).map((_, index) => (
            <LoadingCell key={index} height="32px" width="32px" style={{ borderRadius: '50%', transform: `translateX(-${index * 16}px)` }} />
          ))
        ) : (
          displayedAvatars?.map(({ avatar, address }, index) => (
            <Avatar key={address} src={avatar} address={address} style={{ width: '32px', height: '32px', transform: `translateX(-${index * 16}px)` }} />
          ))
        )}
      </div>
      {isLoading ? (
        ...
        <p className="common-followers-text-container">{displayedNames && formatCommonFollowersText(displayedNames, resultLength)}</p>
      )}
    </div>
  )
}
